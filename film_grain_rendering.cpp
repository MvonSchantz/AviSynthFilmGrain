...
*/
float render_pixel(const float * __restrict const imgIn, const int yOut, const int xOut, const int mIn,
                   const int nIn, const int mOut, const int nOut,
                   const int offset, const int nMonteCarlo,
                   const float grainRadius, const float sigmaR, const float sigmaFilter,
                   const float xA, const float yA, const float xB, const float yB, const float * __restrict const lambdaList,
                   const float * __restrict const expLambdaList, const float * __restrict const xGaussianList, const float * __restrict const yGaussianList)
{
	//float logNormalQuantile;
    const float grainRadiusSq = grainRadius*grainRadius;
    float maxRadius = grainRadius;
    float mu = 0.0;
	float sigma = 0.0;
    float currGrainRadiusSq = grainRadiusSq;

    const float ag = 1/ceil(1/grainRadius);
    const float sX = ((float)(nOut-1))/((float)(xB-xA));
    const float sY = ((float)(mOut-1))/((float)(yB-yA));
    ...
    const float xIn = xA + (xOut+0.5f) * ((xB-xA)/(nOut));
    const float yIn = yA + (yOut+0.5f) * ((yB-yA)/(mOut));

    //calculate the mu and sigma for the lognormal distribution
    if (sigmaR > 0.0f)
    {
	    constexpr float normalQuantile = 3.0902f;
	    sigma = sqrt(log( (sigmaR/grainRadius)*(sigmaR/grainRadius) + 1.0f));
	    const float sigmaSq = sigma * sigma;
        mu = log(grainRadius)-sigmaSq/(2.0f);
        const float logNormalQuantile = exp(mu + sigma*normalQuantile);
    ...
    {
        const float xGaussian = xIn + sigmaFilter*(xGaussianList[i])/sX;
        const float yGaussian = yIn + sigmaFilter*(yGaussianList[i])/sY;

        //determine the bounding boxes around the current shifted pixel
        const int minX = static_cast<int>(floor((xGaussian - maxRadius) / ag));
        const int maxX = static_cast<int>(floor((xGaussian + maxRadius) / ag));
        const int minY = static_cast<int>(floor((yGaussian - maxRadius) / ag));
        const int maxY = static_cast<int>(floor((yGaussian + maxRadius) / ag));
        ...
        for(int ncx = minX; ncx <= maxX; ncx++) /* x-cell number */
        {
            if(ptCovered == true)
                break;
            const float cellCorner_x = ag * ncx;
            for(int ncy = minY; ncy <= maxY; ncy++) /* y-cell number */
            {
                ...
                //cellCorner.x = ag*ncx;
                //cellCorner.y = ag*ncy;
                const float cellCorner_y = ag * ncy;

                /* seed cell = (x/w, y/w) */
                const unsigned int seed = cellseed(ncx,ncy, offset);
                mysrand(&p, seed);

                // Compute the Poisson parameters for the pixel that contains (x,y)
                const float u = imgIn[(int)(fmin(fmax(floor(cellCorner_y),0.0),
                                                    (float)(mIn-1)))*nIn+
                        (int)(fmin(fmax(floor(cellCorner_x),0.0),(float)(nIn-1)))];
                const int uInd = (int)floor(u*((float)(MAX_GREY_LEVEL_8+EPSILON_GREY_LEVEL)));
                const float currLambda = lambdaList[uInd];
                const float currExpLambda = expLambdaList[uInd];

                // Draw number of points in the cell
                const int Ncell = my_rand_poisson(&p, currLambda, currExpLambda);
                for(int k=0; k<Ncell; k++)
                {
                    //draw the grain centre
                    const float xCentreGrain = cellCorner_x + ag*myrand_uniform_0_1(&p);
                    const float yCentreGrain = cellCorner_y + ag*myrand_uniform_0_1(&p);

                    //draw the grain radius
                    if (sigmaR>0.0f)
                    {
                        //draw a random Gaussian radius, and convert it to log-normal
                        const float currRadius = fmin(exp(mu + sigma * myrand_gaussian_0_1(&p)), maxRadius);
    ...
    return(pixOut/static_cast<float>(nMonteCarlo));
}
                                               ...
                                               filmGrainOptionsStruct<float> filmGrainOptions)
{
    ...
    const float grainRadius = filmGrainOptions.muR;//grainRadius/pixelSize;
    const float grainStd = filmGrainOptions.sigmaR;
    const float sigmaFilter = filmGrainOptions.sigmaFilter;

    /*std::cout << "min : " << imgIn->min_() << std::endl;
    std::cout << "max : " << imgIn->max_() << std::endl;*/

    /*********************************************/
    /***MONTE CARLO TRANSLATION VECTORS*****/
    /*********************************************/

    const int NmonteCarlo = filmGrainOptions.NmonteCarlo;
    std::random_device rd;
    std::mt19937 rndGenerator(rd());

    //draw the random (gaussian) translation vectors
    float * __restrict const xGaussianList = new float[NmonteCarlo];
    float * __restrict const yGaussianList = new float[NmonteCarlo];
    ...
    float * __restrict const lambdaList = new float[ MAX_GREY_LEVEL_8 +1];
    float * __restrict const expLambdaList = new float[ MAX_GREY_LEVEL_8 +1];
    for (int i=0; i<=MAX_GREY_LEVEL_8; i++)
    {
        const float u = ((float)i)/( (float) ( (float)MAX_GREY_LEVEL_8 + (float)EPSILON_GREY_LEVEL) );
        const float ag = 1/ceil(1/grainRadius);
        const float lambdaTemp = -((ag*ag) /
    ...
    auto imgOut = new matrix<float>(filmGrainOptions.mOut, filmGrainOptions.nOut);
    /*std::cout << "image output size : " << imgOut->get_ncols();
    std::cout << " x " << imgOut->get_nrows() << std::endl;*/

#pragma omp parallel for schedule(dynamic,2) shared(imgOut)
    for (int i=0; i<(int)imgOut->get_nrows(); i++)
    {
        for (int j=0; j<(int)imgOut->get_ncols(); j++)
        {
            const float* const pt = (float*)imgIn->get_ptr();
            const float pix = render_pixel(pt, i, j, imgIn->get_nrows(),
                               imgIn->get_ncols(),
                               imgOut->get_nrows(), imgOut->get_ncols(),
...
matrix<float>* boolean_model(float lambda, float r, float stdGrain)
{
    ...
    else 	//random radius (with log-normal distribution)
    {
        ...
        float mu = log(r)-sigmaSquare/(2.0f);
